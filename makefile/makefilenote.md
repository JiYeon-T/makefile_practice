...
**大型项目中会嵌套使用很多 Makefile，先看主 Makefile，然后确认大致的构建规则**
...
- patsubst - pattern substitution, 模式字符串替换函数

```makefile
# 1.
dest = $(patsubst <pattern>,<replacement>,<text>)
# 工作原理：%是通配符,可以是任意长度的非空字符串
# 函数会遍历 <text> 中的每一个单词
#如果某个单词符合 <pattern> 中的定义, 就用 <replacement> 替换, 在 <replacement> 中的 % 会被替换为 <pattern> 中 % 的具体内容;
# 1.
dest = $(patsubst <pattern>,<replacement>,<text>)
# # 相当于
...
#e.g. 编译项目中自动推导目标文件列表
SRC = $(wildcard *.c)
OBJ = $(pastsubst %.c, %.o, $(SRC))  # OBJ 变量最终值为:main.o, utils.o, helpler.o,...
DEPS = $(pastsubst %.o, %.d, $(OBJS))  # 为每个文件生成依赖文件:main.d, utils.d, helpler.d,...
#e.g. 2.添加前缀,去除后缀
FILES = a b c  # patsubst 只能处理空格分隔的单词列表
RESULT = $(patsubst %, obj/%, $(FILES))  # 添加前缀
RESULT2 = $(patsubst %.c, %, $(FILES))  # 去除后缀
...
如何包含其他 Make.defs 文件，Make.defs = Makefile.defines
